...
const mappers = {
  ...
  movieLink: config.api.base + '/get/playvideo/',
  seasonExp: /сезон/i,
  digitExp: /\d+/,
...
class Parser {
  ...
  static parseActions(json) {
    ...
    if (containsSeasons) {
      data.childType = 'season';
      data.data = actionsData.seasons.map(s => {
        let obj = {
          id: s.folder + 'season',
          number: s.folder,
          folder: s.folder,
  ...
  /**
   * Gets movie title for search results
   * @param {Object} movie
   * @returns {string}
   */
  static parseSearchMovieTitle(movie) {
    let title = [
      movie.title,
      '(' + movie.year[0] + ')',
      '[' + movie.subsection + ']'
  ...
  static getMovieId(movie) {
    let match = movie.link.match(mappers.urlLastPart);
